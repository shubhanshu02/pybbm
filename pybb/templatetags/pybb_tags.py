...
def pybb_topic_moderated_by(topic, user):  # pragma: no cover
    ...
    """
    warnings.warn(
        "pybb_topic_moderated_by filter is deprecated and will be removed in later releases. "
...
def pybb_editable_by(post, user):  # pragma: no cover
    ...
    """
    warnings.warn(
        "pybb_editable_by filter is deprecated and will be removed in later releases. "
...
def pybb_is_topic_unread(topic, user):
    ...
    unread = not ForumReadTracker.objects.filter(
        forum=topic.forum, user=user.id, time_stamp__gte=last_topic_update
    ).exists()
    unread &= not TopicReadTracker.objects.filter(
        topic=topic, user=user.id, time_stamp__gte=last_topic_update
    ).exists()
    return unread
    ...
    if is_authenticated(user):
        ...
        if len(forum_marks):
            for topic in topic_list:
                ...
                if (
                    topic.forum.id in forum_marks
                    and topic_updated <= forum_marks[topic.forum.id]
                ):
                    topic.unread = False

        qs = TopicReadTracker.objects.filter(
            user=user, topic__in=topic_list
...
def pybb_forum_unread(forums, user):
    ...
    if is_authenticated(user):
        ...
        forum_marks = ForumReadTracker.objects.filter(
            user=user, forum__in=forum_list
        ...
        for mark in forum_marks:
            ...
            if (curr_forum.updated is None) or (curr_forum.updated <= mark.time_stamp):
                if not any(
                    (
                        f.unread
                        for f in pybb_forum_unread(curr_forum.child_forums.all(), user)
                    )
                ):
...
def pybb_topic_poll_not_voted(topic, user):
    if is_anonymous(user):
        return True
    return not PollAnswerUser.objects.filter(
        poll_answer__topic=topic, user=user
    ).exists()
...
def load_perms_filters():
    ...
    for method_name, method in inspect.getmembers(perms):
        ...
        if len(inspect.getargspec(method).args) == 3:
            register.filter(
                "%s%s" % ("pybb_", method_name), partial(method_name, perms)
            )
        elif len(inspect.getargspec(method).args) == 2:
            register.filter(
                "%s%s" % ("pybb_", method_name), partial_no_param(method_name, perms)
            )
