...
if defaults.PYBB_USE_DJANGO_MAILER:
    ...
    def send_html_mail(
        ...
        msg = EmailMultiAlternatives(
            subject, text_msg, sender, recipient, connection=connection
        )
...
def send_mass_html_mail(emails, *args, **kwargs):
    ...
    for email in emails:
        ...
        if html_msg:
            send_html_mail(
                subject, text_msg, html_msg, sender, recipient, *args, **kwargs
            )
        else:
...
def get_paginator_class():
    try:
        ...
        Page.pages = lambda self: [
            PageRepr(i) for i in range(1, self.paginator.num_pages + 1)
        ]
