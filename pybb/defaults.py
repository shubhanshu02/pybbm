...
PYBB_NICE_URL_PERMANENT_REDIRECT = getattr(
    settings, "PYBB_NICE_URL_PERMANENT_REDIRECT", True
)
PYBB_NICE_URL_SLUG_DUPLICATE_LIMIT = getattr(
    settings, "PYBB_NICE_URL_SLUG_DUPLICATE_LIMIT", 100
)

# TODO In a near future, this code will be deleted when callable settings will not supported anymore.
callable_warning = (
    "%(setting_name)s should not be a callable anymore but a path to the parser classes."
    "ex : myproject.markup.CustomBBCodeParser. It will stop working in next pybbm release."
)
wrong_setting_warning = (
    "%s setting will be removed in next pybbm version. "
...
PYBB_MARKUP_ENGINES_PATHS = getattr(
    settings,
...
if not hasattr(settings, "PYBB_MARKUP_ENGINES"):
    ...
    PYBB_MARKUP_ENGINES = getsetting_with_deprecation_check(
...
if not hasattr(settings, "PYBB_QUOTE_ENGINES"):
    ...
    PYBB_QUOTE_ENGINES = getsetting_with_deprecation_check(
...
if not PYBB_MARKUP or PYBB_MARKUP not in PYBB_MARKUP_ENGINES:
    if not PYBB_MARKUP_ENGINES:  # pragma: no cover
        warnings.warn(
            "There is no markup engines defined in your settings. "
        ...
        raise ImproperlyConfigured(
            "PYBB_MARKUP must be defined to an existing key of "
            "PYBB_MARKUP_ENGINES_PATHS"
        )

PYBB_TEMPLATE = getattr(settings, "PYBB_TEMPLATE", "base.html")
PYBB_TEMPLATE_MAIL_TXT = getattr(
    settings, "PYBB_TEMPLATE_MAIL_TXT", "pybb/mail_templates/base.html"
)
PYBB_TEMPLATE_MAIL_HTML = getattr(
    settings, "PYBB_TEMPLATE_MAIL_HTML", "pybb/mail_templates/base-html.html"
)
...
PYBB_ANONYMOUS_VIEWS_CACHE_BUFFER = getattr(
    settings, "PYBB_ANONYMOUS_VIEWS_CACHE_BUFFER", 100
)
...
if not hasattr(settings, "PYBB_BODY_CLEANERS"):
    PYBB_BODY_CLEANERS = [
        "pybb.markup.base.rstrip_str",
        "pybb.markup.base.filter_blanks",
    ]
else:  # pragma: no cover
    PYBB_BODY_CLEANERS = getsetting_with_deprecation_check(
        settings, "PYBB_BODY_CLEANERS"
    )
...
PYBB_PERMISSION_HANDLER = getattr(
    settings, "PYBB_PERMISSION_HANDLER", "pybb.permissions.DefaultPermissionHandler"
)

PYBB_PROFILE_RELATED_NAME = getattr(
    settings, "PYBB_PROFILE_RELATED_NAME", "pybb_profile"
)
...
def _render_quote(name, value, options, parent, context):  # pragma: no cover
    warnings.warn(
        "pybb.defaults._render_quote function is deprecated. "
