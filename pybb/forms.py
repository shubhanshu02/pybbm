...
AttachmentFormSet = inlineformset_factory(
    Post, Attachment, extra=1, form=AttachmentForm
)
...
class BasePollAnswerFormset(BaseInlineFormSet):
    def clean(self):
        forms_cnt = (
            len(self.initial_forms)
            + len([form for form in self.extra_forms if form.has_changed()])
...
class PostForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get("instance", None) and (
            kwargs["instance"].topic.head == kwargs["instance"]
        ):
            kwargs.setdefault("initial", {})["name"] = kwargs["instance"].topic.name
            kwargs.setdefault("initial", {})["poll_type"] = kwargs[
                "instance"
            ].topic.poll_type
            kwargs.setdefault("initial", {})["poll_question"] = kwargs[
                "instance"
            ].topic.poll_question
        ...
        if not (
            self.forum
            or (self.instance.pk and (self.instance.topic.head == self.instance))
        ):
    ...
    def clean(self):
        ...
        if (
            poll_type is not None
            and poll_type != Topic.POLL_TYPE_NONE
    ...
    def save(self, commit=True):
        ...
        if defaults.PYBB_PREMODERATION:
            allow_post = defaults.PYBB_PREMODERATION(
                self.user, self.cleaned_data["body"]
            )
        if self.forum:
            topic = Topic(
                forum=self.forum,
    ...
    def __init__(self, instance, user, *args, **kwargs):
        ...
        if not self.post.is_topic_head:
            ...
            self.posts_to_move = Post.objects.filter(
                created__gte=self.post.created, topic=self.topic
            ).order_by("created", "pk")
            ...
            for post in self.posts_to_move[1:]:  # all except the current one
                i += 1
                bvars = {
                    "author": util.get_pybb_profile(post.user).get_display_name(),
    ...
    def save(self):
        ...
        if not self.post.is_topic_head:
            ...
            if data["number"] != -1:
                number = (
                    data["number"] + 1
                )  # we want to move at least the current post ;-)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "instance" in kwargs and kwargs["instance"]:
            kwargs.setdefault("initial", {}).update(
                {"login": getattr(kwargs["instance"].user, username_field)}
            )
    ...
    def save(self, *args, **kwargs):
        try:
            self.user = User.objects.filter(
                **{username_field: self.cleaned_data["login"]}
            ).get()
...
try:
    ...
    class EditProfileForm(forms.ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fields["signature"].widget = forms.Textarea(
        ...
        def clean_signature(self):
            ...
            if len(re.findall(r"\n", value)) > defaults.PYBB_SIGNATURE_MAX_LINES:
                raise forms.ValidationError(
                    "Number of lines is limited to %d"
                    % defaults.PYBB_SIGNATURE_MAX_LINES
                )
            if len(value) > defaults.PYBB_SIGNATURE_MAX_LENGTH:
                raise forms.ValidationError(
                    "Length of signature is limited to %d"
    ...
    def __init__(self, topic, *args, **kwargs):
        ...
        if topic.poll_type == Topic.POLL_TYPE_SINGLE:
            self.fields["answers"] = forms.ModelChoiceField(
                label="", queryset=qs, empty_label=None, widget=forms.RadioSelect()
            )
        elif topic.poll_type == Topic.POLL_TYPE_MULTIPLE:
            self.fields["answers"] = forms.ModelMultipleChoiceField(
                label="", queryset=qs, widget=forms.CheckboxSelectMultiple()
            )
...
class ForumSubscriptionForm(forms.Form):
    def __init__(self, user, forum, instance=None, *args, **kwargs):
        ...
        if instance:
            type_choices.append(("unsubscribe", _("be unsubscribe from this forum")))
...
class ModeratorForm(forms.Form):
    def __init__(self, user, *args, **kwargs):
        ...
        for category in categories:
            forums = [
                ...
                if permissions.perms.may_change_forum(user, forum)
            ]
            if forums:
    ...
    def process(self, target_user):
        ...
        untouchable_forums = [
            forum
            for forum in initial_forum_set
            if forum.pk not in self.authorized_forums
        ]
