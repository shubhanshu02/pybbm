...
from pybb.views import (
    ...
    post_ajax_preview,
    mark_all_as_read,
    ForumSubscriptionView,
    UserEditPrivilegesView,
)
...
urlpatterns += [
    ...
    re_path(
        r"^users/(?P<username>[^/]+)/topics/$", UserTopics.as_view(), name="user_topics"
    ),
    re_path(r"^users/(?P<username>[^/]+)/posts/$", UserPosts.as_view(), name="user_posts"),
    re_path(
        r"^users/(?P<username>[^/]+)/edit-privileges/$",
    ...
    re_path(r"^topic/(?P<pk>\d+)/stick/$", StickTopicView.as_view(), name="stick_topic"),
    re_path(
        r"^topic/(?P<pk>\d+)/unstick/$",
        UnstickTopicView.as_view(),
        name="unstick_topic",
    ),
    re_path(r"^topic/(?P<pk>\d+)/close/$", CloseTopicView.as_view(), name="close_topic"),
    re_path(r"^topic/(?P<pk>\d+)/open/$", OpenTopicView.as_view(), name="open_topic"),
    re_path(
        r"^topic/(?P<pk>\d+)/poll_vote/$",
        TopicPollVoteView.as_view(),
        name="topic_poll_vote",
    ),
    re_path(
        r"^topic/(?P<pk>\d+)/cancel_poll_vote/$",
        topic_cancel_poll_vote,
    ...
    # Add topic/post
    re_path(
        r"^forum/(?P<forum_id>\d+)/topic/add/$", AddPostView.as_view(), name="add_topic"
    ),
    re_path(r"^topic/(?P<topic_id>\d+)/post/add/$", AddPostView.as_view(), name="add_post"),
    # Post
    ...
    re_path(r"^post/(?P<pk>\d+)/moderate/$", ModeratePost.as_view(), name="moderate_post"),
    # Attachment
    # re_path('^attachment/(\w+)/$', 'show_attachment', name='pybb_attachment'),
    # Subscription
    re_path(
        r"^subscription/topic/(\d+)/delete/$",
        delete_subscription,
        name="delete_subscription",
    ),
    re_path(r"^subscription/topic/(\d+)/add/$", add_subscription, name="add_subscription"),
    re_path(
        r"^subscription/forum/(?P<pk>\d+)/$",
        ForumSubscriptionView.as_view(),
        name="forum_subscription",
    ),
    # API
    re_path("^api/post_ajax_preview/$", post_ajax_preview, name="post_ajax_preview"),
    # Commands
...
if PYBB_NICE_URL:
    urlpatterns += [
        ...
        re_path(
            r"^c/(?P<category_slug>[\w-]+)/(?P<slug>[\w-]+)/$",
            ForumView.as_view(),
            name="forum",
        ),
        re_path(
