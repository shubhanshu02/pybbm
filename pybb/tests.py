...
from pybb.templatetags.pybb_tags import (
    pybb_is_topic_unread,
    pybb_topic_unread,
    pybb_forum_unread,
    pybb_get_latest_topics,
    pybb_get_latest_posts,
)
...
class SharedTestModule(object):
    ...
    def create_post_via_http(
        self, client, forum_id=None, topic_id=None, _sleep=False, **post_kwargs
    ):
        """
    ...
    def create_initial(self, post=True):
        ...
        self.forum = Forum.objects.create(
            name="xfoo", description="bar", category=self.category
        )
        self.topic = Topic.objects.create(
            name="etopic", forum=self.forum, user=self.user
        )
        if post:
...
class FeaturesTest(TestCase, SharedTestModule):
    ...
    def test_base(self):
        ...
        Forum.objects.create(
            name="xfoo1", description="bar1", category=self.category, parent=self.forum
        )
        ...
        self.assertTrue(
            defaults.PYBB_DEFAULT_TITLE in tree.xpath("//title")[0].text_content()
        )
    ...
    def test_profile_language_default(self):
        ...
        self.assertEqual(
            util.get_pybb_profile(user).language,
    ...
    def test_profile_edit(self):
        ...
        response = self.client.post(
            reverse("pybb:edit_profile"), data=values, follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_pagination_and_topic_addition(self):
        ...
        response = self.client.get(url)
        self.assertEqual(
            len(response.context["topic_list"]), defaults.PYBB_FORUM_PAGE_SIZE
        )
        self.assertTrue(response.context["is_paginated"])
        self.assertEqual(
            response.context["paginator"].num_pages,
            int((defaults.PYBB_FORUM_PAGE_SIZE + 3) / defaults.PYBB_FORUM_PAGE_SIZE)
            + 1,
        )
    ...
    def test_topic_read_before_post_addition(self):
        ...
        TopicReadTracker.objects.create(
            user=user_ann, topic=topic, time_stamp=topic.created
        )
    ...
    def test_read_tracking(self):
        ...
        self.assertTrue(
            tree.xpath(
                '//a[@href="%s"]/parent::td[contains(@class,"unread")]'
        ...
        response = self.create_post_via_http(
            client, topic_id=topic.id, body="test tracking", _sleep=True
        )
    ...
    def test_read_tracking_multi_user(self):
        ...
        self.assertEqual(
            TopicReadTracker.objects.filter(user=user_ann, topic=topic_1).count(), 1
        )
        self.assertEqual(ForumReadTracker.objects.all().count(), 0)
        ...
        self.assertEqual(
            TopicReadTracker.objects.filter(user=user_bob, topic=topic_1).count(), 1
        )
        ...
        self.assertEqual(
            ForumReadTracker.objects.filter(user=user_bob, forum=self.forum).count(), 1
        )
    ...
    def test_read_tracking_multi_forum(self):
        ...
        self.assertEqual(
            TopicReadTracker.objects.filter(user=self.user, topic=topic_1).count(), 1
        )
        ...
        self.assertEqual(
            ForumReadTracker.objects.filter(user=self.user, forum=self.forum).count(), 1
        )
    ...
    def test_read_tracker_after_posting(self):
        ...
        self.assertEqual(
            TopicReadTracker.objects.filter(user=self.user, topic=self.topic).count(), 0
        )
        self.assertEqual(
            ForumReadTracker.objects.filter(user=self.user, forum=self.forum).count(), 1
        )
    ...
    def test_pybb_is_topic_unread_filter(self):
        ...
        forum_2 = Forum.objects.create(
            name="forum_2", description="forum2", category=self.category
        )
        ...
        self.assertListEqual(
            [
                t.unread
                for t in pybb_topic_unread([topic_1, topic_2, topic_3], user_ann)
            ],
            [True, True, True],
        )
    ...
    def test_is_forum_unread_filter(self):
        ...
        forum_child1 = Forum.objects.create(
            name="f2", category=self.category, parent=forum_parent
        )
        forum_child2 = Forum.objects.create(
            name="f3", category=self.category, parent=forum_parent
        )
        topic_1 = Topic.objects.create(
            name="topic_1", forum=forum_parent, user=self.user
        )
        topic_2 = Topic.objects.create(
            name="topic_2", forum=forum_child1, user=self.user
        )
        topic_3 = Topic.objects.create(
            name="topic_3", forum=forum_child2, user=self.user
        )
        ...
        self.assertListEqual(
            [
                f.unread
                for f in pybb_forum_unread(
                    [forum_parent, forum_child1, forum_child2], user_ann
                )
            ],
            [True, True, True],
        )
    ...
    def test_read_tracker_when_topics_forum_changed(self):
        forum_1 = Forum.objects.create(
            name="f1", description="bar", category=self.category
        )
        forum_2 = Forum.objects.create(
    ...
    def test_open_first_unread_post(self):
        ...
        response = client_ann.get(
            topic_1.get_absolute_url(), data={"first-unread": 1}, follow=True
        )
        self.assertRedirects(
            response,
            "%s?page=%d#post-%d" % (topic_1.get_absolute_url(), 1, post_1_1.id),
        )
        sleep_only_if_required(1)
        ...
        self.assertRedirects(
            response,
            "%s?page=%d#post-%d" % (topic_1.get_absolute_url(), 1, post_1_2.id),
        )
        sleep_only_if_required(1)

        response = client_ann.get(
            topic_2.get_absolute_url(), data={"first-unread": 1}, follow=True
        )
        self.assertRedirects(
            response,
            "%s?page=%d#post-%d" % (topic_2.get_absolute_url(), 1, post_2_1.id),
        )
        sleep_only_if_required(1)
        ...
        self.assertRedirects(
            response,
            "%s?page=%d#post-%d" % (topic_1.get_absolute_url(), 1, post_1_3.id),
        )
    ...
    def test_latest_topics(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertListEqual(
            list(response.context["topic_list"]), [topic_1, topic_2, topic_3]
        )
        ...
        topic_1.user = User.objects.create_user(
            "another", "another@localhost", "another"
        )
        topic_1.save()
        response = self.client.get(reverse("pybb:topic_latest"))
    ...
    def test_hidden(self):
        ...
        post_hidden = self.create_post(
            topic=topic_hidden, user=self.user, body="hidden"
        )

        post_in_hidden = self.create_post(
            topic=topic_in_hidden, user=self.user, body="hidden"
        )

        self.assertFalse(
            category.id
            in [c.id for c in client.get(reverse("pybb:index")).context["categories"]]
        )
        ...
        self.assertNotContains(
            client.get(reverse("pybb:index")), forum_hidden.get_absolute_url()
        )
        self.assertNotContains(
            client.get(reverse("pybb:feed_topics")), topic_hidden.get_absolute_url()
        )
        self.assertNotContains(
            client.get(reverse("pybb:feed_topics")), topic_in_hidden.get_absolute_url()
        )

        self.assertNotContains(
            client.get(reverse("pybb:feed_posts")), post_hidden.get_absolute_url()
        )
        self.assertNotContains(
            client.get(reverse("pybb:feed_posts")), post_in_hidden.get_absolute_url()
        )
        ...
        response = client.get(
            reverse("pybb:add_post", kwargs={"topic_id": self.topic.id})
        )
        self.assertEqual(response.status_code, 200, response)

        response = client.get(
            reverse("pybb:add_post", kwargs={"topic_id": self.topic.id}),
            data={"quote_id": post_hidden.id},
        )
        ...
        self.assertEqual(
            client.get(forum_in_hidden.get_absolute_url()).status_code, 403
        )
        self.assertEqual(
            client.get(topic_in_hidden.get_absolute_url()).status_code, 403
        )
    ...
    def test_user_blocking(self):
        ...
        response = self.client.get(
            reverse("pybb:block_user", args=[user.username]), follow=True
        )
        ...
        response = self.client.post(
            reverse("pybb:block_user", args=[user.username]),
            data={"block_and_delete_messages": "block_and_delete_messages"},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
        user = User.objects.get(username=user.username)
    ...
    def test_user_unblocking(self):
        ...
        self.login_client()
        response = self.client.get(
            reverse("pybb:unblock_user", args=[user.username]), follow=True
        )
        self.assertEqual(response.status_code, 405)
        response = self.client.post(
            reverse("pybb:unblock_user", args=[user.username]), follow=True
    ...
    def test_ajax_preview(self):
        self.login_client()
        response = self.client.post(
            reverse("pybb:post_ajax_preview"),
            data={"data": "[b]test bbcode ajax preview[/b]"},
        )
    ...
    def test_headline(self):
        ...
        client = Client()
        self.assertContains(
            client.get(self.forum.get_absolute_url()), "test <b>headline</b>"
        )
    ...
    def test_quote(self):
        ...
        response = self.client.get(
            reverse("pybb:add_post", kwargs={"topic_id": self.topic.id}),
            data={"quote_id": self.post.id, "body": "test tracking"},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_stick(self):
        ...
        self.assertEqual(
            self.client.get(
                reverse("pybb:stick_topic", kwargs={"pk": self.topic.id}), follow=True
            ).status_code,
            200,
        )
        self.assertEqual(
            self.client.get(
                reverse("pybb:unstick_topic", kwargs={"pk": self.topic.id}), follow=True
            ).status_code,
            200,
        )
    ...
    def test_delete_view(self):
        ...
        self.login_client()
        response = self.client.post(
            reverse("pybb:delete_post", args=[post.id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
        ...
        # Delete topic
        response = self.client.post(
            reverse("pybb:delete_post", args=[self.post.id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_open_close(self):
        ...
        response = self.client.get(
            reverse("pybb:close_topic", args=[self.topic.id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
        response = self.create_post_via_http(
            client, topic_id=self.topic.id, body="test closed"
        )
        self.assertEqual(response.status_code, 403)
        response = self.client.get(
            reverse("pybb:open_topic", args=[self.topic.id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
        response = self.create_post_via_http(
            client, topic_id=self.topic.id, body="test closed"
    ...
    def test_subscription(self):
        user2 = User.objects.create_user(
            username="user2", password="user2", email="user2@someserver.com"
        )
        ...
        subscribe_links = html.fromstring(response.content).xpath(
            '//a[@href="%s"]' % subscribe_url
        )
        ...
        response = self.create_post_via_http(
            self.client, topic_id=self.topic.id, body="test subscribtion юникод"
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertTrue(
            [msg for msg in mail.outbox if new_post.get_absolute_url() in msg.body]
        )
        ...
        response = client.get(
            reverse("pybb:delete_subscription", args=[self.topic.id]), follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_subscription_disabled(self):
        ...
        user3 = User.objects.create_user(
            username="user3", password="user3", email="user3@someserver.com"
        )
        client = Client()
    ...
    def test_notifications_disabled(self):
        ...
        self.client.login(username="zeus", password="zeus")
        response = self.create_post_via_http(
            client, topic_id=self.topic.id, body="test subscribtion юникод"
        )
        self.assertEqual(response.status_code, 200)
        new_post = Post.objects.order_by("-id")[0]
    ...
    def test_forum_subscription(self):
        ...
        user2 = User.objects.create_user(
            username="user2", password="user2", email="user2@dns.com"
        )
        user3 = User.objects.create_user(
            username="user3", password="user3", email="user3@dns.com"
        )
        client = Client()
        client.login(username="user2", password="user2")
        parser = html.HTMLParser(encoding="utf8")
        ...
        self.assertEqual(
            ["id_type_0", "id_type_1", "id_topics_0", "id_topics_1"], radio_ids
        )
        ...
        tree = html.fromstring(response.content, parser=parser)
        self.assertTrue(
            ["Manage subscription"], tree.xpath('//a[@href="%s"]/text()' % url)
        )
        ...
        usernames = list(
            self.topic.subscribers.all().values_list("username", flat=True)
        )
        self.assertEqual(
            usernames,
            [
        ...
        self.assertEqual(
            "New topic in forum that you subscribed.", mail.outbox[0].subject
        )
        ...
        self.assertEqual(
            "New answer in topic that you subscribed.", mail.outbox[1].subject
        )
        ...
        self.assertEqual(
            tree.xpath('//input[@id="id_type_2"]/@value'),
            [
                "unsubscribe",
            ],
        )
        self.assertEqual(
            tree.xpath('//input[@id="id_type_1"]/@checked'),
            [
                "checked",
            ],
        )
        ...
        usernames = list(
            topic.subscribers.all().order_by("id").values_list("username", flat=True)
        )
        self.assertEqual(
            usernames,
            [
        ...
        tree = html.fromstring(response.content, parser=parser)
        self.assertEqual(
            tree.xpath('//input[@id="id_type_0"]/@checked'),
            [
                "checked",
            ],
        )
        ...
        subscribed_topics = list(
            user2.subscriptions.all().order_by("name").values_list("name", flat=True)
        )
        expected_topics = list(
            self.forum.topics.all().order_by("name").values_list("name", flat=True)
        )
    ...
    def test_topic_updated(self):
        ...
        post = self.create_post(
            _sleep=True, topic=topic, user=self.user, body="bbcode [b]test[/b]"
        )
        client = Client()
        response = client.get(self.forum.get_absolute_url())
    ...
    def test_topic_deleted(self):
        ...
        sleep_only_if_required(1)
        topic_2 = Topic.objects.create(
            name="another topic", forum=forum_1, user=self.user
        )
    ...
    def test_user_views(self):
        response = self.client.get(
            reverse("pybb:user", kwargs={"username": self.user.username})
        )
        self.assertEqual(response.status_code, 200)

        response = self.client.get(
            reverse("pybb:user_posts", kwargs={"username": self.user.username})
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["object_list"].count(), 1)

        response = self.client.get(
            reverse("pybb:user_topics", kwargs={"username": self.user.username})
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["object_list"].count(), 1)
    ...
    def test_post_count(self):
        ...
        post.save()
        self.assertEqual(
            Profile.objects.get(pk=util.get_pybb_profile(self.user).pk).post_count, 2
        )
        post.delete()
        self.assertEqual(
            Profile.objects.get(pk=util.get_pybb_profile(self.user).pk).post_count, 1
        )
    ...
    def test_managing_forums(self):
        ...
        forum2 = Forum.objects.create(
            name="foo2", description="bar2", category=self.category
        )
        Forum.objects.create(name="foo3", description="bar3", category=self.category)
        moderator = User.objects.create_user(
            "moderator", "moderator@localhost", "moderator"
        )
        self.login_client()

        # test the visibility of the button and the access to the page
        response = self.client.get(
            reverse("pybb:user", kwargs={"username": moderator.username})
        )
        self.assertNotContains(
            response,
            '<a href="%s"'
            % reverse("pybb:edit_privileges", kwargs={"username": moderator.username}),
        )
        response = self.client.get(
            reverse("pybb:edit_privileges", kwargs={"username": moderator.username})
        )
        self.assertEqual(response.status_code, 403)
        add_change_forum_permission = Permission.objects.get_by_natural_key(
            "change_forum", "pybb", "forum"
        )
        ...
        inputs = dict(
            html.fromstring(response.content)
            .xpath('//form[@class="%s"]' % "privileges-edit")[0]
            .inputs
        )
        self.assertEqual(
        ...
        available_forums = [
            forum for forum in self.category.forums.all() if not forum.pk % 3 == 0
        ]
        ...
        moderator_list = sorted(
            [forum for forum in moderator.forum_set.all()], key=lambda forum: forum.pk
        )
        self.assertEqual(correct_list, moderator_list)

        # test to remove user as moderator
        values["cat_%d" % self.category.pk] = [
            available_forums[0].pk,
        ]
        response = self.client.post(
            reverse("pybb:edit_privileges", kwargs={"username": moderator.username}),
            data=values,
        ...
        self.assertEqual(
            ...
            [forum for forum in moderator.forum_set.all()],
        )
        values["cat_%d" % self.category.pk] = []
        response = self.client.post(
            reverse("pybb:edit_privileges", kwargs={"username": moderator.username}),
...
class MoveAndSplitPostTest(TestCase, SharedTestModule):
    def create_initial(self):
        ...
        self.forum_1 = Forum.objects.create(
            name="forum_1", category=self.category, position=1
        )
        self.topic = Topic.objects.create(
            name="abc", forum=self.forum_1, user=self.user, views=7
        )
        self.posts = []
        self.posts.append(
            self.create_post(topic=self.topic, user=self.user, body="zero")
        )
        self.posts.append(
            self.create_post(topic=self.topic, user=self.user, body="one")
        )
        self.posts.append(
            self.create_post(topic=self.topic, user=self.user, body="two")
        )
        self.posts.append(
            self.create_post(topic=self.topic, user=self.user, body="three")
        )
        self.posts.append(
            self.create_post(topic=self.topic, user=self.user, body="four")
        )
        self.posts.append(
            self.create_post(topic=self.topic, user=self.user, body="five")
        )

        self.forum_2 = Forum.objects.create(
            name="forum_2", category=self.category, position=2
        )
        self.forum_3 = Forum.objects.create(
            name="forum_3", category=self.category, hidden=True
        )
    ...
    def test_move_topic(self):
        ...
        self.assertEqual(
            topic.views, 8
        )  # +1 because topic is currently viewed by moderator
    ...
    def test_split_posts_all(self):
        ...
        try:
            ...
            self.fail(
                "A new topic in forum 2 should have been created by spliting posts"
            )
    ...
    def test_split_posts_last(self):
        ...
        self.assertEqual(
            self.topic.slug, topic_2.slug
        )  # same slug because not in same forum
    ...
    def test_split_posts_some_same_forum(self):
        ...
        self.assertNotEqual(
            topic_1.slug, topic_2.slug
        )  # can't keep same slug in same forum
...
class AnonymousTest(TestCase, SharedTestModule):
    def setUp(self):
        ...
        self.PYBB_ANONYMOUS_VIEWS_CACHE_BUFFER = (
    ...
    def test_anonymous_posting(self):
        response = self.create_post_via_http(
            self.client, topic_id=self.topic.id, body="test anonymous"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_anonymous_cache_topic_views(self):
        self.assertNotIn(
            util.build_cache_key("anonymous_topic_views", topic_id=self.topic.id), cache
        )
        url = self.topic.get_absolute_url()
        self.client.get(url)
        self.assertEqual(
            cache.get(
                util.build_cache_key("anonymous_topic_views", topic_id=self.topic.id)
            ),
            1,
        )
        ...
        self.client.get(url)
        self.assertEqual(
            Topic.objects.get(id=self.topic.id).views,
            defaults.PYBB_ANONYMOUS_VIEWS_CACHE_BUFFER,
        )
        self.assertEqual(
            cache.get(
...
class PreModerationTest(TestCase, SharedTestModule):
    ...
    def test_premoderation(self):
        ...
        response = self.create_post_via_http(
            self.client, topic_id=self.topic.id, body="test premoderation"
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertRedirects(
            response, settings.LOGIN_URL + "?next=%s" % post.get_absolute_url()
        )
        ...
        user = User.objects.create_user(
            "allowed_zeus", "allowed_zeus@localhost", "allowed_zeus"
        )
        client.login(username="allowed_zeus", password="allowed_zeus")
        response = self.create_post_via_http(
            client, topic_id=self.topic.id, body="test premoderation staff"
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = admin_client.get(
            reverse("pybb:moderate_post", kwargs={"pk": post.id}), follow=True
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = client.get(
            Topic.objects.get(name="new topic name").get_absolute_url()
        )
        self.assertEqual(response.status_code, 302)
        response = admin_client.get(
            reverse(
...
class AttachmentTest(TestCase, SharedTestModule):
    def setUp(self):
        ...
        self.file_name = os.path.join(
            os.path.dirname(__file__), "static", "pybb", "img", "attachment.png"
        )
        self.create_user()
        self.create_initial()
    ...
    def test_attachment_one(self):
        ...
        with open(self.file_name, "rb") as fp:
            response = self.create_post_via_http(
                self.client,
                topic_id=self.topic.id,
                **{"body": "test attachment", "attachments-0-file": fp}
            )
        self.assertEqual(response.status_code, 200)
...
class PollTest(TestCase, SharedTestModule):
    ...
    def test_poll_add(self):
        ...
        response = self.create_post_via_http(
            self.client, forum_id=self.forum.id, **values
        )
        self.assertEqual(response.status_code, 200)
        new_topic = Topic.objects.get(name="test poll name")
        self.assertIsNone(new_topic.poll_question)
        self.assertFalse(
            PollAnswer.objects.filter(topic=new_topic).exists()
        )  # no answers here
        ...
        values[
            "poll_answers-0-text"
        ] = "answer1"  # two answers - what do we need to create poll
        values["poll_answers-1-text"] = "answer2"
        values["poll_answers-TOTAL_FORMS"] = 2
        response = self.create_post_via_http(
            self.client, forum_id=self.forum.id, **values
    ...
    def test_regression_poll_deletion_after_second_post(self):
        ...
        response = self.create_post_via_http(
            self.client, topic_id=new_topic.id, body="test answer body"
        )
        self.assertEqual(PollAnswer.objects.filter(topic=new_topic).count(), 2)
    ...
    def test_poll_edit(self):
        ...
        values[
            "poll_answers-0-text"
        ] = "answer100"  # no matter how many answers we provide
        values["poll_answers-TOTAL_FORMS"] = 1
        response = self.client.post(edit_topic_url, values, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_poll_voting(self):
        ...
        self.assertListEqual(
            [a.votes_percent() for a in PollAnswer.objects.all()], [50.0, 50.0]
        )

        response = self.client.post(vote_url, data=values, follow=True)
        self.assertEqual(response.status_code, 403)  # already voted

        cancel_vote_url = reverse(
            "pybb:topic_cancel_poll_vote", kwargs={"pk": self.topic.id}
        )
        ...
        self.assertListEqual(
            [a.votes_percent() for a in PollAnswer.objects.all()], [0, 0]
        )
...
class CustomPermissionHandler(permissions.DefaultPermissionHandler):
    ...
    def filter_posts(self, user, qs):
        if is_anonymous(user):
            qs = qs.filter(
                Q(topic__forum__hidden=False) & Q(topic__forum__category__hidden=False)
            )
        return qs
...
class MarkupParserTest(TestCase, SharedTestModule):
    ...
    def test_markup_engines(self):
        def _test_engine(parser_name, text_to_html_map):
            for item in text_to_html_map:
                self.assertIn(
                    util._get_markup_formatter(parser_name)(item[0]), item[1:]
                )
        ...
        text_to_html_map = [
            ...
            [
                '[quote="post author"]quote[/quote]',
                "<blockquote><em>post author</em><br>quote</blockquote>",
            ],
            [
                "[code]code[/code]",
    ...
    def test_quote_engines(self):
        def _test_engine(parser_name, text_to_quote_map):
            for item in text_to_quote_map:
                ...
                self.assertEqual(
                    util._get_markup_quoter(parser_name)(item[0], "username"), item[2]
                )
        ...
        text_to_quote_map = [["quote text", "quote text", "quote text"]]
    ...
    def test_body_cleaners(self):
        ...
        cleaners_map = [
            [
                "pybb.markup.base.filter_blanks",
                "some\n\n\n\ntext\n\nwith\nnew\nlines",
                "some\ntext\n\nwith\nnew\nlines",
            ],
            [
...
def _detach_perms_class():
    ...
    """
    pybb_views.perms = permissions.perms = util.resolve_class(
...
class ControlsAndPermissionsTest(TestCase, SharedTestModule):
    def create_initial(
        self, on_moderation=False, closed=False, sticky=False, hidden=False
    ):
        """
        * forum1: normal
            * topic1_1: normal
        ...
        * forum2: hidden
        ...
        forum1 = Forum.objects.create(
            name="forum 1", description="bar 1", category=category
        )
        ...
        if on_moderation:
            self.create_post(
                topic=topic1_1, user=alice, body="post 1_1_3", on_moderation=True
            )
            topic1_2 = Topic.objects.create(
                name="topic 1_2", forum=forum1, user=alice, on_moderation=True
            )
            topics.append(topic1_2)
            self.create_post(
                topic=topic1_2, user=alice, body="post 1_2_1", on_moderation=True
            )
            topic1_3 = Topic.objects.create(
                name="topic 1_3",
                forum=forum1,
                user=alice,
            )
            ...
            self.create_post(
                topic=topic1_3, user=bob, body="post 1_3_2", on_moderation=True
            )
            topic1_3.on_moderation = True
            topic1_3.save()

        if closed:
            topic1_4 = Topic.objects.create(
                name="topic 1_4", forum=forum1, user=alice, closed=True
            )
        ...
        if sticky:
            topic1_5 = Topic.objects.create(
                name="topic 1_5", forum=forum1, user=alice, sticky=True
            )
        ...
        if hidden:
            forum2 = Forum.objects.create(
                name="forum 2", description="bar 2", category=category, hidden=True
            )
    ...
    def test_permission_documentation(self):
        topics = self.create_initial(
            on_moderation=True, closed=True, sticky=True, hidden=True
        )
        ...
        other_on_moderation_topic = Topic.objects.create(
            name="topic 1_6", forum=forum, user=other, on_moderation=True
        )
        self.create_post(
            topic=other_on_moderation_topic,
            user=other,
            body="post 1_6_1",
            on_moderation=True,
        )
        ...
        def _view_own_on_moderation_topic(user):
            if not is_anonymous(user) and author_on_moderation_topic.user.pk == user.pk:
                return permissions.perms.may_view_topic(
                    user, author_on_moderation_topic
                )
        ...
        def _delete_own_on_moderation_post(user):
            if not is_anonymous(user) and author_on_moderation_post.user.pk == user.pk:
                return permissions.perms.may_delete_post(
                    user, author_on_moderation_post
                )
        ...
        # get permissions
        change_topic_perm = Permission.objects.get_by_natural_key(
            "change_topic", "pybb", "topic"
        )
        delete_topic_perm = Permission.objects.get_by_natural_key(
            "delete_topic", "pybb", "topic"
        )
        change_post_perm = Permission.objects.get_by_natural_key(
            "change_post", "pybb", "post"
        )
        delete_post_perm = Permission.objects.get_by_natural_key(
            "delete_post", "pybb", "post"
        )

        # init all users
        anonymous = AnonymousUser()
        redactor = User.objects.create_user(
            "redactor", "redactor", "redactor@localhost"
        )
        redactor.is_staff = True
        redactor.save()
        moderator = User.objects.create_user(
            "moderator", "moderator", "moderator@localhost"
        )
        moderator.save()
        ...
        manager.user_permissions.add(
            change_topic_perm, change_post_perm, delete_topic_perm, delete_post_perm
        )
        superuser = User.objects.create_user(
            "superuser", "superuser", "superuser@localhost"
        )
        ...
        if wrong_parts:
            self.fail(
                "Permission document does not reflect what default Permission handler do. "
    ...
    def test_may_create_post(self):
        ...
        normal_topics = Topic.objects.filter(
            forum__hidden=False,
        ...
        hidden_topics = Topic.objects.filter(
            forum__hidden=True, forum__category__hidden=True
        )
    ...
    def test_filter_posts_author(self):
        ...
        excluded_posts = [
            "post 2_1_2",
        ]  # exclude hidden forum posts I didn't create
        expected_posts = posts.exclude(body__in=excluded_posts)
    ...
    def test_post_actions_superuser(self):
        ...
        response = self.get_with_user(
            topic2.get_absolute_url(), "superuser", "superuser"
        )
        self.assertEqual(response.status_code, 200)
        tree = html.fromstring(response.content)
        hrefs = tree.xpath(
    ...
    def test_post_actions_moderator(self):
        ...
        forum2 = Forum.objects.create(
            name="test 2", description="bar 2", category=forum1.category
        )

        moderator1 = User.objects.create_user(
            "moderator1", "moderator1@localhost", "moderator1"
        )
        moderator2 = User.objects.create_user(
            "moderator2", "moderator2@localhost", "moderator2"
        )
...
class CustomPermissionHandlerTest(TestCase, SharedTestModule):
    ...
    def test_forum_permission(self):
        for f in Forum.objects.all():
            ...
            self.assertEqual(
                r.status_code, 302 if f.hidden or f.category.hidden else 200
            )
            r = self.get_with_user(f.get_absolute_url(), "zeus", "zeus")
            self.assertEqual(r.status_code, 200)
            self.assertEqual(
                r.context["object_list"].count(), f.topics.filter(closed=False).count()
            )
    ...
    def test_topic_permission(self):
        for t in Topic.objects.all():
            ...
            self.assertEqual(
                r.status_code, 302 if t.forum.hidden or t.forum.category.hidden else 200
            )
...
class LogonRedirectTest(TestCase, SharedTestModule):
    ...
    def setUp(self):
        ...
        self.topic.save()
        self.post = self.create_post(
            body="body post", topic=self.topic, user=staff, on_moderation=True
        )
    ...
    def test_redirect_category(self):
        ...
        self.assertRedirects(
            r, settings.LOGIN_URL + "?next=%s" % self.category.get_absolute_url()
        )
    ...
    def test_redirect_forum(self):
        ...
        self.assertRedirects(
            r, settings.LOGIN_URL + "?next=%s" % self.forum.get_absolute_url()
        )
        # access with (unauthorized) user should get 403 (forbidden)
    ...
    def test_redirect_topic(self):
        ...
        self.assertRedirects(
            r, settings.LOGIN_URL + "?next=%s" % self.topic.get_absolute_url()
        )
        # access with (unauthorized) user should get 403 (forbidden)
    ...
    def test_redirect_post(self):
        ...
        self.assertRedirects(
            r, settings.LOGIN_URL + "?next=%s" % self.post.get_absolute_url()
        )
        # access with (unauthorized) user should get 403 (forbidden)
...
class NiceUrlsTest(TestCase, SharedTestModule):
    ...
    def test_unicode_slugify(self):
        self.assertEqual(
            compat.slugify("北京 (China), Москва (Russia), é_è (a sad smiley !)"),
    ...
    def test_no_duplicate_slug(self):
        ...
        forum = Forum.objects.create(
            name=forum_name, description="bar", category=self.category
        )
        ...
        self.assertEqual(
            "%s-%d" % (compat.slugify(category_name), slug_nb), category.slug
        )
        ...
        category = Category.objects.create(
            name="test_slug_category", slug=compat.slugify(category_name)
        )
        forum = Forum.objects.create(
            name="test_slug_forum",
            description="bar",
            category=self.category,
            slug=compat.slugify(forum_name),
        )
        topic = Topic.objects.create(
            name="test_topic_slug",
            forum=self.forum,
            user=self.user,
            slug=compat.slugify(topic_name),
        )
        slug_nb = len(Category.objects.filter(slug__startswith=category_name)) - 1
        self.assertEqual(
            "%s-%d" % (compat.slugify(category_name), slug_nb), category.slug
    ...
    def test_fail_on_too_many_duplicate_slug(self):
        ...
        try:
            ...
            self.fail(
                'Should be able to create "dolly", "dolly-1", ..., "dolly-199".\n'
            )
    ...
    def test_add_topic(self):
        ...
        inputs = dict(
            html.fromstring(response.content)
            .xpath('//form[@class="%s"]' % "post-form")[0]
            .inputs
        )
        self.assertNotIn("slug", inputs)
        values = self.get_form_values(response)
        values.update(
            {"name": self.topic.name, "body": "[b]Test slug body[/b]", "poll_type": 0}
        )
        response = self.client.post(add_topic_url, data=values, follow=True)
        slug_nb = (
            len(Topic.objects.filter(slug__startswith=compat.slugify(self.topic.name)))
            - 1
        )
        self.assertIsNotNone = Topic.objects.get(
            slug="%s-%d" % (self.topic.name, slug_nb)
        )
    ...
    def test_old_url_redirection(self):
        ...
        for redirect_status in [301, 302]:
            ...
            response = self.client.get(
                reverse("pybb:category", kwargs={"pk": self.category.pk})
            )
            self.assertRedirects(
                response, self.category.get_absolute_url(), status_code=redirect_status
            )

            response = self.client.get(
                reverse("pybb:forum", kwargs={"pk": self.forum.pk})
            )
            self.assertRedirects(
                response, self.forum.get_absolute_url(), status_code=redirect_status
            )

            response = self.client.get(
                reverse("pybb:topic", kwargs={"pk": self.topic.pk})
            )
            self.assertRedirects(
                response, self.topic.get_absolute_url(), status_code=redirect_status
            )
...
class TestTemplateTags(TestCase, SharedTestModule):
    ...
    def test_pybb_get_time(self):
        template = Template(
            (
                "{% load pybb_tags %}{% pybb_get_time a_time as time_output %}"
                "{{ time_output|upper }}"
            )
        )
        context = Context(
    ...
    def test_pybb_link(self):
        ...
        expected = (
            ...
            '<a href="%s">%s</a>'
            % (self.post.get_absolute_url(), "Hello &lt;World&gt;"),
        )
    ...
    def test_pybb_posted_by(self):
        ...
        context = Context(
            {"user": self.user, "anonymous": AnonymousUser(), "post": self.post}
        )
        output = template.render(context)
    ...
    def test_pybb_topic_unread(self):
        ...
        for letter in "ABC":
            forum = Forum.objects.create(
                name=letter, description=letter, category=category
            )
            forums.append(forum)
        ...
        output = template.render(context)
        expected = "\n".join(
            ("A1: True", "B1: True", "C1: True", "B2: True", "C2: True", "C3: True", "")
        )
        self.assertEqual(output, expected)
    ...
    def test_pybb_topic_inline_pagination(self):
        ...
        nb_pages = int(
            math.ceil(float(self.topic.post_count) / defaults.PYBB_TOPIC_PAGE_SIZE)
        )
    ...
    def test_pybb_get_profile(self):
        ...
        self.assertEqual(
            template.render(context), "%(pk)d:%(pk)d:None" % {"pk": bob_profile_pk}
        )
    ...
    def test_pybb_get_latest_topics(self):
        ...
        for i in range(1, 10):
            ...
            self.create_post(
                topic=topic, user=self.user, body="foo", on_moderation=bool(i % 2)
            )
    ...
    def test_pybb_get_latest_posts(self):
        ...
        self.post.save()
        self.create_post(
            topic=self.topic, body="B0", user=self.user, on_moderation=True
        )
        for i in range(1, 10):
            topic = Topic.objects.create(
                name="%d" % i,
            ...
            self.create_post(
                topic=topic, body="B%d" % i, user=self.user, on_moderation=True
            )
    ...
    def test_pybbm_calc_topic_views(self):
        ...
        bob = User.objects.create_user("bob", "bob@localhost", "bob")
        self.get_with_user(
            self.topic.get_absolute_url(), username="bob", password="bob"
        )
    ...
    def build_test_method(method_name, method, required_arg, templatetag_name):
        if method_name.startswith("filter") and len(method_args) == 3:
            ...
            def test(self):
                ...
                expected = "".join(
                    ["%s" % obj for obj in method(self.user, context["qs"])]
                )
    ...
    for method_name, method in inspect.getmembers(permissions.perms):
        ...
        if len(method_args) == 3:
            ...
            if required_arg not in arg_mapping:
                ...
                continue
        test_method = build_test_method(
            method_name, method, required_arg, templatetag_name
        )
        if test_method:
...
class MiscTest(TestCase, SharedTestModule):
    def test_profile_avatar_url_property(self):
        ...
        path = os.path.join(
            os.path.dirname(__file__), "static", "pybb", "img", "image.png"
        )
        profile.avatar = SimpleUploadedFile(
            name="image.png", content=open(path, "rb").read(), content_type="image/png"
        )
        profile.save()
        self.assertNotEqual(profile.avatar_url, defaults.PYBB_DEFAULT_AVATAR_URL)
        self.assertTrue(
            profile.avatar_url.startswith(settings.MEDIA_URL + "pybb/avatar/")
        )
